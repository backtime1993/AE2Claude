...
t('layer_exists',       lambda: call('layer_exists', 'Text1'))
t('get_layer_info',     lambda: call('get_layer_info', 'Text1'))
t('list_comps',         lambda: call('list_comps'))

# 2. Property Read (6)
t('get_position',       lambda: call('get_transform_value', 'Text1', 'position'))
t('get_opacity',        lambda: call('get_transform_value', 'Text1', 'opacity'))
t('get_pos_at_t',       lambda: call('get_transform_value', 'Text1', 'position', '--at_time', '0.5'))
t('get_keyframes',      lambda: call('get_keyframes', 'Text1', 'opacity'))
t('get_prop_val',       lambda: call('get_property_value', 'Text1', 'property("ADBE Transform Group").property("ADBE Opacity")'))
t('get_prop_kfs',       lambda: call('get_property_keyframes', 'Text1', 'property("ADBE Transform Group").property("ADBE Position")'))
...
t('add_mask',           lambda: call('add_mask_rect', 'Solid1', '50', '50', '400', '300', '--feather', '5'))
...
# 11. Keyframes (5)
t('pos_kf',             lambda: call('set_position_keyframes', 'Solid1', '[[0,[100,100]],[1,[500,500]]]'))
t('opa_kf',             lambda: call('set_opacity_keyframes', 'Solid1', '[[0,0],[0.5,100],[1,0]]'))
t('scale_kf',           lambda: call('set_scale_keyframes', 'Solid1', '[[0,[50,50,100]],[1,[100,100,100]]]'))
t('rot_kf',             lambda: call('set_rotation_keyframes', 'Solid1', '[[0,0],[1,360]]'))
t('easing',             lambda: call('apply_easing', 'Solid1', '["opacity","position","scale"]'))

# 12. Effects (4)
t('add_blur',           lambda: call('add_effect', 'Solid1', 'gaussian_blur', '--props', '{"blurriness": 20}'))
t('add_shadow',         lambda: call('add_effect', 'Solid1', 'drop_shadow', '--props', '{"opacity": 150}'))
t('enum_fx',            lambda: call('enumerate_effects', 'Solid1'))
t('fx_ease',            lambda: call('apply_effect_easing', 'Solid1', 'gaussian_blur', 'blurriness'))
...
# 17. Selection (3)
t('select',             lambda: call('select_layers', '["Text1","Solid1"]'))
t('get_selected',       lambda: call('get_selected_layers'))
t('deselect',           lambda: call('deselect_all'))

# 18. Batch (1)
t('batch',              lambda: call('batch_jsx', '["\\\"a\\\"","1+2","\\\"b\\\""]'))
