...
if (parentComp == NULL) {
	...
	err = suites.CompSuite11()->AEGP_CreateCameraInComp(utf16Name.data(), centerPoint, parentComp, &newCameraLayerH);
...
Result<AEGP_LayerH> CreateLightInComp(const std::string& name, float x, float y, Result<AEGP_CompH> parentCompH) {
	...
	auto utf16Name = convertUTF8ToUTF16(name);
	err = suites.CompSuite11()->AEGP_CreateLightInComp(utf16Name.data(), centerPoint, parentComp, &newLightLayerH);
...
if (parentComp == NULL) {
	...
	auto utf16Name = convertUTF8ToUTF16(name);
	err = suites.CompSuite11()->AEGP_CreateNullInComp(utf16Name.data(), parentComp, &duration, &newNullLayerH);
