...
if (proj == nullptr) {
	...
	auto utf16Path = convertUTF8ToUTF16(path);
	err = suites.ProjSuite6()->AEGP_SaveProjectToPath(proj, utf16Path.data());
...
Result<void> SaveProjectAs(Result<AEGP_ProjectH> projH, const std::string& path) {
	...
	auto utf16Path2 = convertUTF8ToUTF16(path);
	err = suites.ProjSuite6()->AEGP_SaveProjectAs(proj, utf16Path2.data());
...
Result<AEGP_ProjectH> OpenProjectFromPath(const std::string& path) {
	...
	auto utf16Path = convertUTF8ToUTF16(path);
	err = suites.ProjSuite6()->AEGP_OpenProjectFromPath(utf16Path.data(), &projectH);
