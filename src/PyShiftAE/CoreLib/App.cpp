...
Project App::getProject() {
	...
	if (result.error != A_Err_NONE) {
		throw std::runtime_error("Error getting active project");
	}
...
void App::beginUndoGroup(std::string undoName) {
	auto& message = enqueueSyncTaskQuiet(StartUndoGroup, undoName);
	message->wait();
...
void App::endUndoGroup() {
	auto& message = enqueueSyncTaskQuiet(EndUndoGroup);
	message->wait();
...
{
	auto& message = enqueueSyncTaskQuiet(ExecuteCommand, commandId);
	message->wait();
...
void App::reportInfo(std::string info) { //should modify to accept string, int, float, etc.
	auto& message = enqueueSyncTaskQuiet(ReportInfo, info);
	message->wait();
...
{
	auto& message = enqueueSyncTaskQuiet(getPluginPaths);
	message->wait();
