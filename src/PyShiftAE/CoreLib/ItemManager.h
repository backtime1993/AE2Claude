...
public:
    ...
    // Phase 3 — CompItem property expansion
    std::vector<float> getBGColor();
    void setBGColor(float r, float g, float b);
    float getWorkAreaStart();
    float getWorkAreaDuration();
    void setWorkArea(float start, float duration);
    ...
    static std::shared_ptr<CompItem> CreateNew(std::string name, float width, float height, float frameRate, float duration, float aspectRatio) {
        ...
        auto& message = enqueueSyncTaskQuiet(Addcomp, name, width, height, frameRate, duration, aspectRatio, itemHandle);
        ...
        auto& message2 = enqueueSyncTaskQuiet(GetItemFromComp, result);
        message2->wait();
...
public:
    ...
    std::vector<std::shared_ptr<Item>> append(std::shared_ptr<Item> item) {
        ...
        auto& message2 = enqueueSyncTaskQuiet(SetItemParentFolder, itemhandle2, itemhandle);
        message2->wait();
...
public:
    ...
    static std::shared_ptr<FolderItem> createNew(std::string name) {
        ...
        auto& projectItem = enqueueSyncTaskQuiet(getProjectRootFolder);
        projectItem->wait();
        Result<AEGP_ItemH> projItem = projectItem->getResult();

        auto& createfootage = enqueueSyncTaskQuiet(createFolderItem, name, projItem);
		createfootage->wait();
...
// Constructors and Destructors
	...
	static std::shared_ptr<SolidItem> createNew(std::string name, float width, float height, float red, float green, float blue, float alpha, float duration, int index = -1) {
		...
		auto& createfootage = enqueueSyncTaskQuiet(NewSolidFootage, name, width, height, red, green, blue, alpha);
		createfootage->wait();
...
public:
	...
	static std::shared_ptr<AdjustmentLayer> createNew(std::shared_ptr<CompItem> compH, std::string name = "Adjustment Layer") {
        ...
        auto& compHOut = enqueueSyncTaskQuiet(getCompFromItem, itemHandle);
        compHOut->wait();
		...
		auto& message = enqueueSyncTaskQuiet(CreateSolidInComp, name, comp.getWidth(), comp.getHeight(), 0, 0, 0, 0, compH2, comp.getDuration());
		message->wait();
        ...
        auto& message2 = enqueueSyncTaskQuiet(SetLayerFlag, result, flag, true);
        message2->wait();
