...
Result<AEGP_ProjectH> ResolveProjectHandle(const Result<AEGP_ProjectH>& projectHandle)
{
	if (projectHandle.error == A_Err_NONE && projectHandle.value != nullptr) {
		return projectHandle;
	}

	auto& message = enqueueSyncTaskQuiet(getProject);
	message->wait();
...
std::shared_ptr<Item> Project::ActiveItem() {
	auto& message = enqueueSyncTaskQuiet(getActiveItem);
	message->wait();
...
{
	auto& message = enqueueSyncTaskQuiet(ActiveLayer);
	message->wait();
...
{
	auto proj = ResolveProjectHandle(this->projH_);
	if (proj.error != A_Err_NONE || proj.value == nullptr) {
		throw std::runtime_error("Error getting project");
	}
	auto& message = enqueueSyncTaskQuiet(GetProjectName, proj);
	message->wait();
...
std::string Project::getPath()
{
	...
	auto& message = enqueueSyncTaskQuiet(GetProjectPath, proj);
	message->wait();
...
void Project::saveAs(std::string path)
{
	...
	auto& message = enqueueSyncTaskQuiet(SaveProjectAs, proj, path);
	message->wait();
...
std::shared_ptr<ProjectCollection> Project::SelectedItems()
{
	...
	for (auto item : items) {
		...
		auto& message = enqueueSyncTaskQuiet(IsItemSelected, itemH);
		message->wait();
...
std::vector<std::shared_ptr<Item>> ProjectCollection::getItems()
{
	...
	auto& itemResult = enqueueSyncTaskQuiet(getProjectRootFolder);
	itemResult->wait();
	...
	auto& itemMessage = enqueueSyncTaskQuiet(GetFirstProjItem, projectResult);
	itemMessage->wait();
